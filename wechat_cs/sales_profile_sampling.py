...
def _repair_profile_coverage(
    ...
    *,
    ...
    while True:
        ...
        _rank, stratum, replacement, replaceable = min(options, key=lambda item: item[0])
...
def _repair_birthday_coverage(
    ...
    *,
    ...
    selected_keys = {item.customer_key for _stratum, item in selected}
    selected_phones = {item.phone_hmac for _stratum, item in selected}
    while sum(item.birthday_match for _stratum, item in selected) < MIN_BIRTHDAY_MATCHES:
...
def select_sales_profile_subjects(
    ...
    *,
    ...
    _repair_profile_coverage(selected, rows, secret=secret)
    _repair_birthday_coverage(selected, rows, secret=secret)
