...
- 首次真实 Kimi Code canary 暴露出数组/对象和字段名漂移；提取与综合 Prompt 升级为 v2，内嵌严格 JSON 输出契约，旧 Prompt run 保留但禁止继续生成。
- 首张结构通过的 canary 又暴露出脱敏占位符被当作商品属性、无证据宣称售后已处理；综合 Prompt 升级为 v3，模型事实会清除占位符，并硬拒绝无证据的售后完成话术。
- 事件必须引用真实 `message_key` 或 `order_line_id`；不存在、原文不匹配或数值不一致的证据直接拒绝。
- 作战卡固定包含：客户价值、商品偏好、时间节律、购买驱动力、历史承诺、当前机会、建议联系理由、自然开场、风险、未知项和证据。
- 综合 Prompt v4 带入客服 SOP 与当前业务口径：默认厂家持续供货，但具体颜色/尺码、实时商品和报价仍由人工确认；微信报价以 40% 毛利为默认基准，30%-40% 区间、老客户例外、退货率优先选品和成本平局规则只作为人工决策指导。客户爽快度、价格敏感和议价倾向必须由历史证据支持，不能使用贬义标签，也不能编造清仓、稀缺、发票原因、库存或优惠。
- 超过 24,000 字或 300 条消息时按时间切片，不拆开同一会话轮次。
- Open Platform 提取温度 `0`、综合温度 `0.2`；用户后续明确要求直接使用 Kimi Code 会员 Key，因此 `kimi-for-coding` 兼容路径按服务端要求固定温度 `1`。并发数 2；仅对 408、429、5xx 和网络超时最多重试 3 次。
...
## 2026-07-16：CPA GPT 独立扩批

- 保留正式 Kimi 50 人批次 `sales-profile-run_cb162cd9b6ce9c532b1c8e59`，新批次明确排除其中 50 位客户。
- 新批次冻结 100 人，使用 `provider=cliproxyapi`、`model=gpt-5.5`，provider、模型、配额和排除批次写入运行配置与哈希。
- 分层配额为复杂风险 10、未来回访/等待 10、高频 25、高价值 25、沉睡复购 20、普通对照 10；未来回访层不超过剩余真实候选量。
- 先执行 3 人 canary，再处理剩余 pending 项。生成结果仍不得自动发送，不覆盖原批次画像或人工审核。
- 后续全量使用 `all_remaining=true` 的独立 run，排除前述 50+100 人并按手机号去重；不伪造分层配额，继续复用现有人工审核和无发送边界。
